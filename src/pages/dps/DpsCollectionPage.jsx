...
const DpsCollectionPage = () => {
  ...
  // Fetch member's DPS settings when member selected
  useEffect(() => {
  ...
  // Auto fill collectedAmount when scheme selected
  useEffect(() => {
    const scheme = settings.find((s) => s.schemeId._id === form.schemeId);
    if (scheme) {
      setForm((prev) => ({ ...prev, collectedAmount: scheme.schemeId.monthlyAmount }));
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/dps-collections`, form);
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            <select name="schemeId" value={form.schemeId} onChange={handleChange} className="w-full border px-3 py-2 rounded-lg" required>
              ...
              {settings.map((s) => <option key={s._id} value={s.schemeId._id}>{s.schemeId.schemeName}</option>)}
            </select>
          </div>
          ...
          <button type="submit" className="w-full bg-indigo-500 hover:bg-indigo-600 text-white py-2 rounded-lg font-semibold mt-4">কালেকশন করুন</button>
        </form>
      </div>
    </div>
  );
